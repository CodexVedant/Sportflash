...
type Props = NativeStackScreenProps<RootStackParamList, 'UpcomingMatches'>;

export default function UpcomingMatchesScreen({ navigation }: Props) {
    const [selectedDate, setSelectedDate] = useState(new Date().toISOString().split('T')[0]);
    const [selectedSport, setSelectedSport] = useState('all');

    const dates = useMemo(() => getNextSevenDays(), []);

    // Explicitly define possible types for sport query
    ...
    // groupedMatches seems unused in the TSX below but was calculated in JS version. 
    // The render loop uses `filteredMatches.map(renderMatchCard)`. 
    // If the design intended grouped by date, it's not fully implemented in the render block shown.
    // Keeping it for now.
        ...
        return (
            <SafeAreaView style={styles.container} edges={['top']}>
                <View style={styles.emptyContainer}>
                    ...
                    <Text style={styles.emptyText}>{(error as any).message || 'Something went wrong'}</Text>
                    <TouchableOpacity
                        onPress={refetch}
