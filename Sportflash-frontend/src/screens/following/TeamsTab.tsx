...
export default function TeamsTab() {
    ...
    const rawFavoriteTeams = user?.preferences?.favoriteTeams || []; // This might be (string | Team)[]

    // Deduplicate and Validate teams
    const favoriteTeams = useMemo(() => {
        const seen = new Set();
        return rawFavoriteTeams.filter((team: any) => {
    ...
    // Logic to extract matches array
    const matches = Array.isArray(rawMatches) ? rawMatches : (rawMatches as any)?.data || [];

    const loadData = useCallback(() => {
        if (!favoriteTeams.length) return;

        const payloadTeams = favoriteTeams.map((t: any) => {
            if (typeof t === 'string') return { id: t, sport: 'football' }; // Default sport if unknown
            return t;
        });

        // Backend expects { teams: [...] }
    ...
    const handleUnfollow = async (teamData: any) => {
        try {
            // Construct new list of IDs
            // We assume backend expects IDs for update
            const currentIds = rawFavoriteTeams.map((t: any) => typeof t === 'string' ? t : t.id);
            const newIds = currentIds.filter((id: string) => String(id) !== String(teamData.id));

            // Optimistic update via Redux thunk
            await dispatch(updateUserPreferences({ favoriteTeams: newIds })).unwrap();

            showToast(`Unfollowed ${teamData.name || 'Team'}`, 'success');
        } catch (error) {
            console.error(error);
            showToast("Failed to unfollow team", 'error');
        }
    };

    const renderTeamItem = ({ item }: { item: any }) => {
        const teamData = typeof item === 'string' ? { id: item, name: 'Unknown Team', sport: 'football' } : item;
        const nextMatch = getNextMatch(String(teamData.id));
            ...
            >
                ...
                >
                    <View style={styles.cardContent}>
                        ...
                        {/* Right: Star (Clickable to Unfollow) */}
                        <TouchableOpacity
                            onPress={() => handleUnfollow(teamData)}
                        ...
                        {nextMatch ? (
                            <>
                                ...
                                <Text style={styles.dateText}>
                                    {nextMatch.date || nextMatch.startTime} {nextMatch.time ? `• ${nextMatch.time}` : ''}
                                </Text>
                            </>
                        ) : (
    ...
    return (
        <View style={styles.container}>
            <FlatList
                ...
                keyExtractor={(item) => (typeof item === 'string' ? item : item.id)?.toString() || Math.random().toString()}
